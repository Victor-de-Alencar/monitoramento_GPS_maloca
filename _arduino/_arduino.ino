...
const char* server = "https://chartreuse-meteor-breadfruit.glitch.me"; // URL do servidor
...
#define limite 0.001
float iniciallat = 2.836446;
float iniciallon = -60.691231;
bool stop = true;
...
void loop() {
  ...
  if (newData && gps.location.isValid()) {
    ...
    if(stop == true) {  // Usando == para comparação
    if(lat > (iniciallat + limite) || lat < (iniciallat - limite) || lon > (iniciallon + limite) || lon < (iniciallon - limite)) {
        outBounds = true;
    } else {
        outBounds = false;
    }
    ...
    String url = String(server) + "/checkbounds?out=" + String(outBounds);  // Usando String() para converter o valor booleano
}
